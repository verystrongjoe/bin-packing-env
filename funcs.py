...
def playMatchesBetweenVersions(env, run_version, player1version, player2version, EPISODES, logger, turns_until_tau0, goes_first = 0):
    ...
    if player1version == -1:
        ...
        player1_NN = GenModel(config.REG_CONST, config.LEARNING_RATE, env.input_shape,   env.action_size).to(config.device)
    ...
    if player2version == -1:
        ...
        player2_NN = GenModel(config.REG_CONST, config.LEARNING_RATE, env.input_shape,   env.action_size).to(config.device)
